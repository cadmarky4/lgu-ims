...
const MemberRow: React.FC<{ 
  ...
  if (!resident) {
    return (
      <tr className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
        <td className="px-4 py-3 border-b" colSpan={4}>
          <span className="text-red-500 text-sm">Resident not found (ID: {memberId})</span>
        </td>
      </tr>
    );
  }
    ...
    <tr className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
      ...
      <td className="px-4 py-3 border-b">
        <div className="text-sm text-gray-900">
          {resident.mobile_number || resident.landline_number || 'No contact'}
        </div>
      </td>
      <td className="px-4 py-3 border-b">
        <select
        ...
        {fields.length === 0 && (
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-gray-600 text-center text-sm">
              No members added yet. Search and select residents to add as household members.
            </p>
          </div>
        )}
